...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/fetch-image?folder=aloys').then((res) => res.json()),
    ...
    fetch('/api/front?folder=aloys_front_image')
      .then((r) => r.json())
      .then((data: string[]) => {
      ...
      <section className="relative w-full h-screen flex items-center justify-center overflow-hidden bg-black">
        ...
        {!isLaptopView && frontImages.length > 0 && (
          ...
          >
            ...
            >
              <img
                src={`/aloys_front_image/${frontImages[currentImageIndex]}`}
                alt="Featured"
...
/>
      ...
      {showPlusIcon && (
        <motion.a
          ...
          className="fixed right-10 bottom-10 p-4 bg-gradient-to-r from-orange-400 to-red-600 text-white rounded-full shadow-xl transition-all hover:scale-105"
        >
          <svg
